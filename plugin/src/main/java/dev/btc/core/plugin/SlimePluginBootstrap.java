...
public class SlimePluginBootstrap implements PluginBootstrap {
    ...
    public void bootstrap(BootstrapContext bootstrapContext) {
        if(!ServerBuildInfo.buildInfo().isBrandCompatible(Key.key("btc-core", "btccore"))) {

            ComponentLogger logger = bootstrapContext.getLogger();
            logger.error("================================================");
            logger.error("BTC-Core Plugin - Incompatible Server");
            logger.error("================================================");
            logger.error("");
            logger.error("It looks like you're trying to run the BTC-Core (ASP) plugin");
            logger.error("on a server that is NOT using the BTC-Core software.");
            logger.error("");
            logger.error("The ASP plugin only works with the BTC-Core server jar.");
            logger.error("Running it on PaperMC or other forks will not work.");
            logger.error("");
            logger.error("To fix this, replace your current server jar");
            logger.error("with a BTC-Core compatible jar from our website:");
            logger.error("https://btc-core.com");
            logger.error("");
