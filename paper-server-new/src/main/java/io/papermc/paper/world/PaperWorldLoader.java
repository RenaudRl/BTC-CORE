...
public record PaperWorldLoader(MinecraftServer server, String levelId) {
    ...
    public void loadInitialWorlds() {
        ...
        for (final LevelStem stem : registry) {
            ...
            if(levelStemKey == LevelStem.NETHER && dev.btc.core.SlimeNMSBridgeImpl.instance().loadNetherOverride()) {
                continue;
            }
            if(levelStemKey == LevelStem.END && dev.btc.core.SlimeNMSBridgeImpl.instance().loadEndOverride()) {
                continue;
            }
            if(levelStemKey == LevelStem.OVERWORLD && dev.btc.core.SlimeNMSBridgeImpl.instance().loadOverworldOverride()) {
                continue;
            }
