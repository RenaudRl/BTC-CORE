...
sourceSets {
    ...
    test {
        java { srcDir("../paper-server-new/src/test/java") }
        resources { srcDir("../paper-server-new/src/test/resources") }
    }
}
val log4jPlugins = sourceSets.create("log4jPlugins") {
    java { srcDir("../paper-server-new/src/log4jPlugins/java") }
}
...
val runtimeConfiguration = configurations.create("runtimeConfiguration") {
    isCanBeConsumed = true
    isCanBeResolved = false
    attributes.attribute(Usage.USAGE_ATTRIBUTE, objects.named(Usage.JAVA_RUNTIME))
    extendsFrom(configurations.getByName(sourceSets.getByName("main").runtimeElementsConfigurationName))
}
...
dependencies {
    ...
    implementation("io.netty:netty-transport-native-epoll:4.1.118.Final:linux-x86_64")
    implementation("io.netty:netty-transport-native-epoll:4.1.118.Final:linux-aarch_64")
...
paperweight {
    ...
    val btccore = forks.register("btccore") {
        upstream.patchDir("paperServer") {
            upstreamPath = "paper-server"
            excludes = setOf("src/minecraft", "patches", "build.gradle.kts")
            patchesDir = rootDirectory.dir("btccore-server/paper-patches")
            outputDir = rootDirectory.dir("paper-server-new")
        }
        upstream.patchDir("paperApi") {
            upstreamPath = "paper-api"
            excludes = setOf("src/minecraft", "patches", "build.gradle.kts")
            patchesDir = rootDirectory.dir("btccore-api/paper-patches")
            outputDir = rootDirectory.dir("paper-api-new")
        }
    }
...
tasks.jar {
    manifest {
        ...
        val buildTime = providers.environmentVariable("BUILD_STARTED_AT").map(Instant::parse).orElse(Instant.now()).get()
        ...
        try {
            ...
            gitHash = git.exec(providers, "rev-parse", "--short=7", "HEAD").get().trim()
            date = git.exec(providers, "show", "-s", "--format=%ci", gitHash).get().trim()
            gitBranch = git.exec(providers, "rev-parse", "--abbrev-ref", "HEAD").get().trim()
        } catch (e: Exception) {
            println("Warning: Failed to obtain git information for manifest: ${e.message}")
        }
...
val scanJarForBadCalls = tasks.register("scanJarForBadCalls", io.papermc.paperweight.tasks.ScanJarForBadCalls::class) {
...
val generatedDir: java.nio.file.Path = rootProject.layout.projectDirectory.dir("paper-server-new/src/generated/java").asFile.toPath()
idea {
    module {
...
fun TaskContainer.registerRunTask(
    ...
    // jvmArgs("-XX:+AllowEnhancedClassRedefinition") // Removed for Java 25 compatibility with Adoptium

    val pluginJar = rootProject.project(":plugin").tasks.named<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("shadowJar").flatMap { it.archiveFile }
    inputs.file(pluginJar)
    args("-add-plugin=${pluginJar.get().asFile.absolutePath}")
