...
public class Converter {
    ...
    public static <T extends BinaryTag> Tag convertTag(T tag) {
        try {
            ...
            LOGGER.error("Failed to convert NBT object: {}", tag);
            throw e;
        }
    }
    ...
    public static <T extends BinaryTag> T convertTag(Tag base) {
        return switch (base.getId()) {
            ...
            case Tag.TAG_STRING -> (T) StringBinaryTag.stringBinaryTag(base.asString().orElse("")); // ASP - Safely fallback to empty string if somehow missing
            case Tag.TAG_LIST -> {
