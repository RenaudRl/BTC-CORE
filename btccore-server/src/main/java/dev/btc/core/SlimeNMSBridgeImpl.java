...
import dev.btc.core.level.SlimeBootstrap;
import dev.btc.core.level.SlimeInMemoryWorld;
...
public class SlimeNMSBridgeImpl implements SlimeNMSBridge {
    ...
    private static final Logger LOGGER = LogManager.getLogger("SlimeNMSBridge");
    ...
    public boolean loadOverworldOverride() {
        ...
        SlimeLevelInstance instance = ((SlimeInMemoryWorld) this.loadInstance(defaultWorld, Level.OVERWORLD))
                .getInstance();
        DimensionDataStorage worldpersistentdata = instance.getDataStorage();
        ((org.bukkit.craftbukkit.CraftServer) Bukkit.getServer()).scoreboardManager = new org.bukkit.craftbukkit.scoreboard.CraftScoreboardManager(
                instance.getServer(), instance.getScoreboard());
    ...
    public void setDefaultWorlds(SlimeWorld normalWorld, SlimeWorld netherWorld, SlimeWorld endWorld) {
        if (normalWorld != null) {
            normalWorld.getPropertyMap().setValue(SlimeProperties.ENVIRONMENT,
                    World.Environment.NORMAL.toString().toLowerCase());
            defaultWorld = normalWorld;
        }

        if (netherWorld != null) {
            netherWorld.getPropertyMap().setValue(SlimeProperties.ENVIRONMENT,
                    World.Environment.NETHER.toString().toLowerCase());
            defaultNetherWorld = netherWorld;
        }

        if (endWorld != null) {
            endWorld.getPropertyMap().setValue(SlimeProperties.ENVIRONMENT,
                    World.Environment.THE_END.toString().toLowerCase());
    ...
    private SlimeLevelInstance createCustomWorld(SlimeWorld world, @Nullable ResourceKey<Level> dimensionOverride) {
        ...
        ResourceKey<Level> worldKey = dimensionOverride == null
                ...
                : dimensionOverride;
        LevelStem stem = MinecraftServer.getServer().registries().compositeAccess().lookupOrThrow(Registries.LEVEL_STEM)
                .get(dimension).orElseThrow().value();
        ...
        try {
            ...
            LOGGER.error("Failed to initialize SlimeLevelInstance for world {}: {}", worldName, ex.getMessage());
            throw new RuntimeException("Severe error during custom world creation for " + worldName, ex);
        }
    ...
    private PrimaryLevelData createWorldData(SlimeWorld world) {
        ...
        // BTCCore start - Apply default game rules from config
        GameRuleMap gameRuleMap = GameRuleMap.of();
        GameRules gameRules = new GameRules(context.dataConfiguration().enabledFeatures(), gameRuleMap);

        // Apply configured defaults
        dev.btc.core.config.SlimeWorldConfig.getInstance().applyRules(worldName, gameRules);
        // BTCCore end

        LevelSettings worldsettings = new LevelSettings(worldName, serverProps.gameMode.get(), false,
                serverProps.difficulty.get(),
                true, gameRules, mcServer.worldLoaderContext.dataConfiguration());
