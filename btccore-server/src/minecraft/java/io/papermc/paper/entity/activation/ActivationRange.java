...
public final class ActivationRange {
    ...
    private static void activateEntity(final Entity entity, final Player player) {
        if (net.minecraft.server.MinecraftServer.currentTick > entity.activatedTick) {
            ...
            if (dev.btc.core.config.BTCCoreConfig.dearEnabled && entity.getType().dabEnabled) {
                if (!entity.activatedPriorityReset) {
                    entity.activatedPriorityReset = true;
                    entity.activatedPriority = dev.btc.core.config.BTCCoreConfig.dearMaxTickFreq;
                }
                ...
                entity.activatedPriority = squaredDistance > dev.btc.core.config.BTCCoreConfig.dearStartDistanceSquared ?
                        Math.min(dev.btc.core.config.BTCCoreConfig.dearMaxTickFreq, (squaredDistance / dev.btc.core.config.BTCCoreConfig.dearActivationDistMod)) : 1;
            } else {
