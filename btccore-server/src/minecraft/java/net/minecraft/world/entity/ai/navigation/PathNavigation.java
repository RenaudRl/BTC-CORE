...
public abstract class PathNavigation {
    ...
    // BTCCore start - Async Pathfinding
    ...
    protected @Nullable Path createPath(Set<BlockPos> targets, @Nullable Entity target, int regionOffset, boolean offsetUpward, int reachRange, float followRange) {
        ...
        if (targets.isEmpty()) {
            ...
            // BTCCore start - Async Pathfinding Execution
            if (dev.btc.core.config.BTCCoreConfig.asyncPathfindingEnabled && this.level instanceof ServerLevel) {
    ...
    public void tick() {
        // BTCCore start - Async Pathfinding Wait
        if (this.futurePath != null) {
