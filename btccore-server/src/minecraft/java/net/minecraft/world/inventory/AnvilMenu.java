    ...
    protected void onTake(Player player, ItemStack stack) {
        ...
        this.access.execute((level, blockPos) -> {
            ...
            if (!player.hasInfiniteMaterials() && blockState.is(BlockTags.ANVIL) && player.getRandom().nextFloat() < 0.12F) {
                ...
                com.destroystokyo.paper.event.block.AnvilDamagedEvent event = new com.destroystokyo.paper.event.block.AnvilDamagedEvent(getBukkitView(), blockState1 != null ? org.bukkit.craftbukkit.block.data.CraftBlockData.createData(blockState1) : null);
                if (!event.callEvent()) {
                    return;
